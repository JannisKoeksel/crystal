...
import { OrderByNode } from "../operations/OrderByNode.js";
import { GroupByNode } from "../operations/GroupByNode.js";
    ...
    PgSelectQueryBuilder
{
  ...
  // GROUP BY
  private groupByNode: GroupByNode | null = null;

  // HAVING
  private havingNodes: HavingNode[] = [];

  // ORDER BY
  private isOrderUnique = false;

  //NEW ORDERBY
  private orderByNode: OrderByNode | null = null;
  ...
  where(whereNode: NodeOrCallback<WhereNode>): void {
    if (this.locker.locked) {
      throw new Error(
  ...
  having(havingNode: NodeOrCallback<HavingNode>): void {
    ...
    const node = getNode(havingNode);

    if (HavingNode.is(node)) {
      this.havingConditions.push(node);
    } else {
      //@ts-expect-error
  ...
  orderBy(orderNode: NodeOrCallback<OrderByNode>): void {
    this.locker.assertParameterUnlocked("orderBy");

    const node = getNode(orderNode);

    if (OrderByNode.is(node)) {
      this.ordering = node;
    } else {
      //@ts-expect-error
      const never: never = node.kind;
